...
private:
  ...
  template <class P> static constexpr bool LayoutCompatible() {
    return std::is_standard_layout<P>() && sizeof(P) == sizeof(Pair) &&
           ...
           OffsetOf<P>::kFirst == OffsetOf<Pair>::kFirst &&
...
private:
  ...
  mutable gtl::aligned_storage_t<sizeof(slot_type), alignof(slot_type)>
      slot_space_;
};
...
#else
  #define GTL_THROW_IMPL_MSG(e, message)                                       \
    do {                                                                       \
      (void)(message);                                                         \
      std::abort();                                                            \
    } while (0)
...
static inline void ThrowStdOutOfRange(const std::string &what_arg) {
  GTL_THROW_IMPL_MSG(std::out_of_range, what_arg);
}
