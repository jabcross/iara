...
struct FirstLastSemaphoreMap {
  ...
  void arrive(i64 id, i64 this_resources, i64 total_resources,
    ...
    auto onFirstToArrive = [&first_args, &every_time_args, &last_args,
                            _this = this, this_resources = this_resources,
                            &erase](Entry &entry) {
      _this->first_time_func(first_args, entry.data);
      erase = detail::common(*_this, entry, every_time_args, last_args);
    };

    auto onSecondOnwards = [&every_time_args, &last_args, _this = this,
                            this_resources = this_resources,
                            &erase](Entry &entry) {
    ...
    map.lazy_emplace_l(id, /*if there is already an entry*/
                       onSecondOnwards, onFirstToArrive);
