...
void populateDeallocEdgeData(EdgeOp edge, StaticAnalysisData &data) {
  ...
  dealloc_edge_info.cons_beta = -1;
  dealloc_edge_info.prod_alpha = last_edge_info.cons_alpha;
  dealloc_edge_info.prod_beta = last_edge_info.cons_beta;
  dealloc_edge_info.delay_offset = 0;
  dealloc_edge_info.delay_size = 0;
  dealloc_edge_info.first_chunk_size = last_edge_info.first_chunk_size;
  dealloc_edge_info.next_chunk_sizes = last_edge_info.next_chunk_sizes;
}
...
void annotateDeallocations(SmallVector<NodeOp> &dealloc_nodes,
  ...
  for (auto dealloc_node : dealloc_nodes) {
    ...
    assert(rem == 0);
    dealloc_node_info.total_firings = alpha_firing + beta_firings;
  }
}
...
void annotateAllocations(SmallVector<Value> &vals, StaticAnalysisData &data) {
  for (auto val : vals) {
    ...
    assert(rem == 0);
    alloc_node_info.total_firings = alpha_firing + beta_firings;
  }
}
