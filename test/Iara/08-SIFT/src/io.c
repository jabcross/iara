...
void write_rgb2ppm(const char *filename, unsigned char *r, unsigned char *g,
                   unsigned char *b, int w, int h, int tot_image_size) {
...
void setPixelRed(unsigned char *img_r, unsigned char *img_g,
                 unsigned char *img_b, int w, int h, int r, int c) {
  size_t index = r * w + c;
  if ((r >= 0) && (r < h) && (c >= 0) && (c < w)) {
    img_r[index] = 0;
    img_g[index] = 0;
...
void export_kpt_list_to_file(const char *filename, int nb_kpt,
  ...
  if (bIncludeDescriptor) {
    for (size_t index = 0; index < nb_kpt; ++index) {
      ...
      fprintf(fp, "%d\t%d\t%.2f\t%.2f\t%.2f\t%.2f\t", kpt->octave, kpt->layer,
              kpt->r, kpt->c, kpt->scale, kpt->ori);
...
void export_keypoints_to_key_file(int FilePathLength, int nKeypointsMax,
                                  ...
                                  IN char *filename, IN SiftKpt *keypoints,
                                  IN int *nbKeypoints) {
#ifdef SIFT_DEBUG
  ...
  export_kpt_list_to_file(newFileName, *nbKeypoints, keypoints, DumpDescriptor,
                          nBins);
}

void read_pgm(int FilePathLength, int image_width, int image_height, int tim,
              IN char *filename, OUT unsigned char *img) {
  ...
  if (image_width != w || image_height != h) {
    fprintf(stderr, "Image size parameters are not corresponding the image "
                    "loaded, leaving now.\n");
    fprintf(stderr, "Found: %dx%d, expected: %dx%d", w, h, image_width,
            image_height);
    exit(1);
  }
...
void rasterCircle(unsigned char *img_r, unsigned char *img_g,
                  unsigned char *img_b, int w, int h, int r, int c,
                  int radius) {
  int f = 1 - radius;
  int ddF_x = 1;
...
void draw_red_orientation(unsigned char *img_r, unsigned char *img_g,
                          unsigned char *img_b, int w, int h, int x, int y,
                          float ori, int cR) {
...
int combine_image(unsigned char *out_image, int w1, int h1,
                  unsigned char *image1, int w2, int h2,
...
void draw_keypoints_to_ppm_file(int FilePathLength, int image_height,
                                int image_width, int nKeypointsMax,
                                int tot_image_size,
  ...
  for (int index = 0; index < *nbKeypoints; ++index) {
    ...
    float ori = kpt->ori;
    draw_red_orientation(img_r, img_g, img_b, image_width, image_height, c, r,
                         ori, cR);
  }
  ...
  write_rgb2ppm(newFileName, img_r, img_g, img_b, image_width, image_height,
                tot_image_size);
}

// Draw a line on the RGB color image.
int draw_line_to_rgb_image(unsigned char *data, int w, int h,
                           struct MatchPair *mp, int offset) {
  int r1 = mp->r1;
  int c1 = mp->c1;
  int r2 = mp->r2;
  int c2 = mp->c2 + offset;
...
int draw_match_lines_to_ppm_file(const char *filename, unsigned char *image1,
                                 int w1, int h1, unsigned char *image2, int w2,
                                 int h2, int nb_match,
...
void draw_red_circle(unsigned char *img_r, unsigned char *img_g,
                     unsigned char *img_b, int w, int h, int r, int c, int cR) {
...
void draw_circle(unsigned char *img_r, unsigned char *img_g,
                 unsigned char *img_b, int w, int h, int r, int c, int cR,
                 float thickness) {
  int x, y;
  int cR2 = cR * cR;
