...
void SPLIT_upsample2x(int image_width, int imgDouble, int image_height,
  ...
  if (imgDouble) {
    ...
    // useless with ne version of upsample
    /* else { */
    /*   memcpy(in + image_width*image_height, in + image_width*(image_height -
     * 1), sizeof(float)*image_width); */
    /* } */
  }
...
void upsample2x(int image_height, int imgDouble, int image_width,
  ...
  if (imgDouble) {
    ...
    int end_line = (*iter == parallelismLevel - 1) ? chunckSize - ceilf(scale)
                                                   : chunckSize;
    ...
    for (int r = 0; r < end_line; r++) {
      ...
      for (int c = 0; c < end_col; c++) {
        ...
        int idx = r1imgW + c1;
        img2x[rdstW + c] = /* (unsigned char) */ (
            (1 - dr) * (1 - dc) * img[idx] +
            dr * (1 - dc) *
                (r1 < image_height - 1 ? img[idx + image_width] : img[idx]) +
            (1 - dr) * dc * img[idx + 1] +
            dr * dc *
                ((r1 < image_height - 1) ? img[idx + image_width + 1]
                                         : img[idx]));
      }
    ...
    if (*iter == parallelismLevel - 1) {
      for (int r = end_line; r < end_line + ceilf(scale); r++) {
        ...
        for (int c = 0; c < end_col; c++) {
          ...
          int idx = r1imgW + c1;
          img2x[rdstW + c] = /* (unsigned char) */ (
              (1 - dr) * (1 - dc) * img[idx] + dr * (1 - dc) * img[idx] +
              (1 - dr) * dc * img[idx + 1] + dr * dc * img[idx]);
        }
        for (int c = end_col; c < dstW; c++) {
