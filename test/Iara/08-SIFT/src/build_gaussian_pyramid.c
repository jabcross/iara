...
void row_filter_transpose(float *src, float *dst, float *row_buf, int w, int h,
                          float *coef1d, int gR) {
...
void row_filter_transpose_1(int image_height, int image_width, int nGpyrLayers,
  ...
  row_filter_transpose(src, imgGT, row_buf, image_width,
...
void row_filter_transpose_2(int image_height, int image_width, int nGpyrLayers,
  ...
  row_filter_transpose(img, imgGT, row_buf, image_height,
                       image_width / parallelismLevel,
                       gaussian_coefs + gWmax * (*iter), gR);
}
...
void row_filter_transpose2x_1(int image_height, int image_width, int imgDouble,
  ...
  if (imgDouble) {
    ...
    row_filter_transpose(src, imgGT, row_buf, 2 * image_width,
                         2 * image_height / parallelismLevel,
                         gaussian_coefs + gWmax * (*iter), gR);
  }
}
...
void row_filter_transpose2x_2(int image_height, int image_width, int imgDouble,
  ...
  if (imgDouble) {
    ...
    row_filter_transpose(img, imgGT, row_buf, 2 * image_height,
                         2 * image_width / parallelismLevel,
                         gaussian_coefs + gWmax * (*iter), gR);
  }
}
...
void seq_blur1(int image_width, int image_height, int nGpyrLayers, int gWmax,
  ...
  row_filter_transpose(src, tmp, row_buf, image_width / 2, image_height / 2,
                       gaussian_coefs + gWmax * (*iter), gR);
  row_filter_transpose(tmp, imgBlurred, row_buf, image_height / 2,
                       image_width / 2, gaussian_coefs + gWmax * (*iter), gR);

  /* char fgpyr[512]; */
  /* unsigned char buffer[tot_image_size/4]; */
  /* sprintf(fgpyr, "seq_blur1-%d.pgm", *iter); */
  /* write_float_pgm(fgpyr, imgBlurred, buffer, image_width/2, image_height/2,
   * 1); */
}
...
void seq_blurN(int image_width, int image_height, int nGpyrLayers, int gWmax,
  ...
  int gR = column_sizes[*iter];
  float *src = *iter ? imgBlurPrev : fst_img;
  int dstW = image_width >> ((*octaveLevel) + 2),
      dstH = image_height >> ((*octaveLevel) + 2);
  ...
  row_filter_transpose(src, tmp, row_buf, dstW, dstH,
                       gaussian_coefs + gWmax * (*iter), gR);
  row_filter_transpose(tmp, imgBlurred, row_buf, dstH, dstW,
                       gaussian_coefs + gWmax * (*iter), gR);
...
void MERGE_gpyr(int nGpyrLayers, int imgDouble, int nOctavesDownN,
  ...
  if (imgDouble) {
    ...
    /*     write_float_pgm(ff2x, gpyrsUp2x, buffer, image_width*2,
     * image_height*2, 1); */
    /* #endif */
...
void compute_gaussian_coefs(int gWmax, int nGpyrLayers, int nLayers,
                            int imgDouble, OUT int *column_sizes,
