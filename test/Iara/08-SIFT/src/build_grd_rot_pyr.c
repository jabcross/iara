...
void ITERATOR_build_grd_rot_pyr(int parallelismLevel, int nOctaves, int nLayers,
                                ...
                                int imgDouble, OUT int *start_octave,
...
void build_grd_rot_pyr(int totSizeWithoutLayers, int parallelismLevel,
   ...
   * grdPyr, *start_octave, *stop_octave, *start_layer, *stop_layer,
    ...
    for (int j = begin_layer; j < end_layer; j++) {
      ...
      for (int r = begin_line; r < end_line; r++) {
        ...
        for (int c = begin_col; c < end_col; c++) {
          float dr = get_pixel_f(srcData, w, h, r + 1, c) -
                     get_pixel_f(srcData, w, h, r - 1, c);
          float dc = get_pixel_f(srcData, w, h, r, c + 1) -
        ...
        /*       if (begin_line == 0 && r == h - 1 && begin_col == 0 && end_col
         * == w) { */
