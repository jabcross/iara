...
void ITERATOR_detect_keypoints(int parallelismLevel, int nOctaves,
                               int image_width, int image_height, int imgDouble,
                               int nLayers, OUT int *start_octave,
...
#endif
  ITERATOR_generic(parallelismLevel, nOctaves, nLayers, image_width,
                   image_height, imgDouble, start_octave, stop_octave,
                   start_layer, stop_layer, start_line, stop_line, start_col,
                   stop_col);
}

// Compute orientation histogram for keypoint detection.
// using pre-computed gradient information.
float compute_orientation_hist_with_gradient(int w, int h, float *grdImage,
                                             float *rotImage,
...
#define TMPHIST(idx)                                                           \
  (idx < 0 ? tmpHist[0]                                                        \
           : (idx >= SIFT_ORI_HIST_BINS ? tmpHist[SIFT_ORI_HIST_BINS - 1]      \
                                        : tmpHist[idx]))
  ...
  hist[SIFT_ORI_HIST_BINS - 2] =
      (tmpHist[SIFT_ORI_HIST_BINS - 4] + tmpHist[SIFT_ORI_HIST_BINS - 1]) *
          1.0f / 16.0f +
      (tmpHist[SIFT_ORI_HIST_BINS - 3] + tmpHist[SIFT_ORI_HIST_BINS - 1]) *
          4.0f / 16.0f +
      tmpHist[SIFT_ORI_HIST_BINS - 2] * 6.0f / 16.0f;
  hist[SIFT_ORI_HIST_BINS - 1] =
      (tmpHist[SIFT_ORI_HIST_BINS - 3] + tmpHist[SIFT_ORI_HIST_BINS - 1]) *
          1.0f / 16.0f +
      (tmpHist[SIFT_ORI_HIST_BINS - 2] + tmpHist[SIFT_ORI_HIST_BINS - 1]) *
          4.0f / 16.0f +
...
bool refine_local_extrema(float *dogPyr, int w, int h, int nDogLayers,
                          int imgDouble, struct SiftKeypoint *kpt) {
  ...
  if (tmp_layer < 0 || tmp_layer > nDogLayers || tmp_r < 0 || tmp_r > h - 1 ||
      tmp_c < 0 || tmp_c > w - 1)
    return false;

  float value =
      get_pixel_f(currData, w, h, r, c) + 0.5f * (dx * xc + dy * xr + ds * xs);
...
int compareKeypointThreshold(struct SiftKeypoint *kpt_in_list,
                             struct SiftKeypoint *kpt_to_add) {
...
void detect_keypoints(int totSizeWithoutLayers, int parallelismLevel,
  ...
  /* #ifdef SIFT_DEBUG */
  /*   if (*start_octave == 0 && *start_layer == 0 && *start_line == 0 &&
   * *start_col == 0) { */
  /*     char fd[512]; */
  /*     int w = image_width; */
  /*     int h = image_height; */
   ...
   * *stop_octave, *start_layer, *stop_layer, *start_line, *stop_line,
   * *start_col, *stop_col); */
  for (int i = *start_octave; i < *stop_octave; i++) {
    ...
    for (int j = begin_layer; j < end_layer; j++) {
      ...
      int begin_line =
          (i == *start_octave) && (j - 1 == *start_layer) ? *start_line : 0;
      int end_line =
          (i + 1 == *stop_octave) && (j == *stop_layer) ? *stop_line : h;
      for (int r = tmax_i(SIFT_IMG_BORDER, begin_line);
           r < tmin_i(end_line, h - SIFT_IMG_BORDER); r++) {

        int begin_col = (i == *start_octave) && (j - 1 == *start_layer) &&
                                (r == *start_line)
                            ? *start_col
                            : 0;
        int end_col = (i + 1 == *stop_octave) && (j == *stop_layer) &&
                              (r + 1 == *stop_line)
                          ? *stop_col
                          : w;

        size_t indexTmp = r * w;
        for (int c = tmax_i(SIFT_IMG_BORDER, begin_col);
             c < tmin_i(end_col, w - SIFT_IMG_BORDER); c++) {
          size_t index = indexTmp + c;
          float val = currData[index];

          bool bExtrema =
              (val >= contr_thr && val > highData[index - w - 1] &&
               val > highData[index - w] && val > highData[index - w + 1] &&
               val > highData[index - 1] && val > highData[index] &&
               val > highData[index + 1] && val > highData[index + w - 1] &&
               val > highData[index + w] && val > highData[index + w + 1] &&
               val > currData[index - w - 1] && val > currData[index - w] &&
               val > currData[index - w + 1] && val > currData[index - 1] &&
               val > currData[index + 1] && val > currData[index + w - 1] &&
               val > currData[index + w] && val > currData[index + w + 1] &&
               val > lowData[index - w - 1] && val > lowData[index - w] &&
               val > lowData[index - w + 1] && val > lowData[index - 1] &&
               val > lowData[index] && val > lowData[index + 1] &&
               val > lowData[index + w - 1] && val > lowData[index + w] &&
          ...
          if (bExtrema) {
            ...
            bool bGoodKeypoint = refine_local_extrema(
                currData, w, h, nDogLayers, imgDouble, &kpt);
            ...
            float max_mag = compute_orientation_hist_with_gradient(
                w, h, grdPyr + offset_gpyr, rotPyr + offset_gpyr, &kpt, hist);
            ...
            for (int ii = 0; ii < SIFT_ORI_HIST_BINS; ii++) {
...
#else
              // Use 3 points to fit a curve and find the accurate location of a
              // keypoints
              ...
              if (currHist > lhist && currHist > rhist &&
                ...
                // Since bin index means the starting point of a bin, so the
                // real orientation should be bin index plus 0.5. for example,
                // angles in bin 0 should have a mean value of 5 instead of 0;
                accu_ii += 0.5f;
                accu_ii = accu_ii < 0 ? (accu_ii + SIFT_ORI_HIST_BINS)
                          : accu_ii >= SIFT_ORI_HIST_BINS
                              ? (accu_ii - SIFT_ORI_HIST_BINS)
                              : accu_ii;
                // The magnitude should also calculate the max number based on
                // fitting But since we didn't actually use it in image
                // matching, we just lazily use the histogram value.
                kpt.mag = currHist;
  ...
  // last keypoint is void and is used to store the number of keypoints detected
  // in the octave attribute
  ...
  /*   fprintf(stderr, "(all) %d: %d\t%d\t%f\t%f\n", i, keypoints[i].octave,
   * keypoints[i].layer, keypoints[i].ri, keypoints[i].ci); */
  /* } */
  /* fprintf(stderr, "\n"); */
}
