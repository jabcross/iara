...
static int check_factorization(
  ...
  dlacpy_(&ALL, &N, &N, A1, &LDA, Residual, &N, 1);

  /* Dealing with L'L or U'U  */
  if (uplo == 'U') {
    dlacpy_(&UP, &N, &N, A2, &LDA, L1, &N, 1);
    dlacpy_(&UP, &N, &N, A2, &LDA, L2, &N, 1);
    dtrmm_(&LO, &uplo, &TR, &NU, &N, &N, &alpha, L1, &N, L2, &N);
  } else {
    dlacpy_(&LO, &N, &N, A2, &LDA, L1, &N, 1);
    dlacpy_(&LO, &N, &N, A2, &LDA, L2, &N, 1);
  ...
  double Rnorm = dlange_(&NORM, &N, &N, Residual, &N, work, 1);
  double Anorm = dlange_(&NORM, &N, &N, A1, &N, work, 1);
