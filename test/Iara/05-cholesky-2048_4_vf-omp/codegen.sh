...
# Generators are now in the experiment-framework directory
FRAMEWORK_DIR=$(dirname "$PATH_TO_APP_SOURCES")/experiment-framework

python $FRAMEWORK_DIR/generate_split_join.py $MATRIX_SIZE $NUM_BLOCKS > kernel_split_join.inc.h

python $FRAMEWORK_DIR/generate_topology.py $MATRIX_SIZE $NUM_BLOCKS > topology.mlir
