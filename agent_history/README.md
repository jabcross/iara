...
This is an example of an out-of-tree [MLIR](https://mlir.llvm.org/) dialect along with a iara `opt`-like tool to operate on that dialect.
...
cmake -G Ninja `$LLVM_SRC_DIR/llvm` \
    ...
    -DLLVM_EXTERNAL_PROJECTS=iara-dialect -DLLVM_EXTERNAL_IARA_DIALECT_SOURCE_DIR=../
cmake --build . --target check-iara
```
