...
pwd ;\
cmake -GNinja $LLVM_DIR/llvm -DLLVM_ENABLE_PROJECTS="mlir;clang" -DCLANG_ENABLE_CIR=ON -DLLVM_BUILD_EXAMPLES=ON -DLLVM_TARGETS_TO_BUILD="host" -DCMAKE_BUILD_TYPE=Release -DLLVM_ENABLE_ASSERTIONS=ON -DLLVM_INSTALL_UTILS=ON -DLLVM_ENABLE_RTTI=ON -DLLVM_CCACHE_BUILD=1 -DLLVM_PARALLEL_LINK_JOBS=1 -DLLVM_USE_LINKER=mold -DCMAKE_EXPORT_COMPILE_COMMANDS=YES ;\
