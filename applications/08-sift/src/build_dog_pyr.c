...
void ITERATOR_build_dog_pyr(OUT int *start_octave, OUT int *stop_octave,
...
#endif
  // Use standard SIFT configuration (from sift_config.h)
  ...
  ITERATOR_generic(parallelismLevel, nOctaves, nDogLayers, image_width,
...
void build_dog_pyr(int nDogLayers, int totSizeWithoutLayers,
   ...
   * dstData, *start_octave, *stop_octave, *start_layer, *stop_layer,
  ...
  for (int i = *start_octave; i < *stop_octave; i++) {
    ...
    for (int j = begin_layer; j < end_layer; j++) {
      int begin_line =
          (i == *start_octave) && (j == *start_layer) ? *start_line : 0;
      int end_line =
          (i + 1 == *stop_octave) && (j + 1 == *stop_layer) ? *stop_line : h;

      int begin_col =
          (i == *start_octave) && (j == *start_layer) ? *start_col : 0;
      int end_col =
          (i + 1 == *stop_octave) && (j + 1 == *stop_layer) ? *stop_col : w;
      ...
      /* fprintf(stderr, "index = %lu\toctave: %d\tlayer: %d\tline: %d-%d\tcol:
       * %d-%d\n", (unsigned long) dstData, i, j, begin_line, end_line,
       * begin_col, end_col); */
