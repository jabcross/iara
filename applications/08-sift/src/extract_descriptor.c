...
void MERGE_keypoints(int nKeypointsMax, int parallelismLevel, int nLocalKptMax,
  ...
  for (int i = 0; i < parallelismLevel; i++) {
    ...
    memcpy(keypoints_out + offset_out, keypoints_in + offset_in,
           nbKpt * sizeof(struct SiftKeypoint));
...
void extract_descriptor(
  ...
  // (rbin, cbin, obin) means (row of hist bin, column of hist bin, orientation
  // bin) In Lowe's paper, 4x4 histogram, each has 8 bins. that means for each
  // (rbin, cbin), there are 8 bins in the histogram.
  ...
  float exp_scale =
      -2.0f / (nSubregion * nSubregion); // -1/(2* nSubregion/2 * nSubregion/2)
  ...
  for (int index = 0; index < nbKeypoints; index++) {
    ...
    /* if (octave == 0 && layer == 1 && ((int) kpt->r) == 8 && ((int) kpt->c) ==
     * 307) { */
    /*   for (int i = 0; i < 15; i++) { */
    ...
    for (int i = top; i <= bottom; i++) {   // rows
      for (int j = left; j <= right; j++) { // columns
        ...
        // Since for a bin array with 4x4 bins, the center is actually at
        // (1.5, 1.5)
        rbin = rrotate + nHalfSubregion - 0.5f;
        cbin = crotate + nHalfSubregion - 0.5f;

        // rbin, cbin range is (-1, d); if outside this range, then the pixel is
        // counted.
        if (rbin <= -1 || rbin >= nSubregion || cbin <= -1 ||
            cbin >= nSubregion)
          continue;

        // All the data need for gradient computation are valid, no border
        // issues.
        r = kptr_i + i;
        c = kptc_i + j;
        rw = r * w;
        ...
        float angle1 =
            (angle < 0) ? (_2PI + angle) : angle; // Adjust angle to [0, 2PI)
  ...
  if (/* nbKeypoints_out != NULL &&  */ keypoints_out != NULL) {
    ...
    memcpy(keypoints_out, keypoints_in,
           (nLocalKptMax + 1) * sizeof(struct SiftKeypoint));
  }
}
