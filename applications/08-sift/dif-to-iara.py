...
def format_tensor_type(rate: int, _type: str) -> str:
    if _type.startswith("tensor"):
        inner = _type.removeprefix("tensor<").removesuffix(">")
        # Find the first 'x' separator between rate and type
        # Use a simple split that handles the first x only
        parts = inner.split('x', 1)  # Split on first 'x' only
        old_rate = int(parts[0])
        old_type = parts[1] if len(parts) > 1 else parts[0]
        rate *= old_rate
    ...
    def to_iara(self) -> None:
        ...
        for block in self.graph_blocks:
            for datatype in block.datatypes:
                # Define the actual LLVM struct type for SiftKpt instead of a placeholder
                if datatype.name == "SiftKpt":
                    # SiftKpt struct: octave(i32), layer(i32), rlayer(f32), r(f32), c(f32), scale(f32),
                    #                ri(f32), ci(f32), layer_scale(f32), ori(f32), mag(f32), descriptors(array<128 x f32>)
                    datatypes[datatype.name] = Datatype("!llvm.struct<(i32, i32, f32, f32, f32, f32, f32, f32, f32, f32, f32, array<128 x f32>)>", 1)
                else:
        ...
        for block in self.graph_blocks:
            ...
            class NodeOp:
                ...
                def format_input_ports(self) -> str:
                    if len(self.in_ports) == 0:
                        return ''
                    # Format as: in ( %val:type, %val2:type2 )
                    port_strs = []
                    for in_port in self.in_ports:
                        port_name = "%"+str(in_port.get_ssa_val_name())
                        port_type = format_tensor_type(in_port.rate, translate_type(in_port.type))
                        port_strs.append(f'{port_name}:{port_type}')
                    return ' in ( ' + ', '.join(port_strs) + ' )'

                def format_output_types(self) -> str:
                    if len(self.out_ports) == 0:
                        return ''
                    # Format as: out ( type1, type2 )
                    type_strs = []
                    for out_port in self.out_ports:
                        assert (type(out_port.rate) is not None)
                        port_type = format_tensor_type(out_port.rate, translate_type(out_port.type))
                        type_strs.append(port_type)
                    return ' out ( ' + ', '.join(type_strs) + ' )'
            ...
            for port in self.get_production_ports(block.name):
                ...
                print(
                    f'    iara.out ( %{port.name}_o_i : {_type} )',)
