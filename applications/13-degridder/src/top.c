...
void config_struct_set_up(Config *config_struct) {
  ...
  case 1:
    config_struct->visibility_source_file = DATA_ROOT_PATH "/sim_small.ms";
    config_struct->input = DATA_ROOT_PATH "/input/cycle_0_deconvolved_2560.csv";
    break;

  case 2:
    config_struct->input = DATA_ROOT_PATH "/input/cycle_0_deconvolved_2560.csv";
    config_struct->visibility_source_file = DATA_ROOT_PATH "/sim_medium.ms";
    break;

  case 3:
    config_struct->input = DATA_ROOT_PATH "/input/cycle_0_deconvolved_5120.csv";
    config_struct->visibility_source_file = DATA_ROOT_PATH "/sim_large.ms";
    break;
  ...
  config_struct->degridding_kernel_support_file =
      DATA_ROOT_PATH "/input/kernels/small/w-proj_supports_x16_2458_image.csv";
  ...
  config_struct->degridding_kernel_imag_file =
      DATA_ROOT_PATH "/input/kernels/small/w-proj_kernels_imag_x16_2458_image.csv";
  config_struct->degridding_kernel_real_file =
      DATA_ROOT_PATH "/input/kernels/small/w-proj_kernels_real_x16_2458_image.csv";
...
void load_image_from_file(Config *config, float *output) {
  if (config == NULL) {
    fprintf(stderr, "FATAL ERROR: config pointer is NULL in load_image_from_file.\\n");
    exit(EXIT_FAILURE);
  }
  if (config->input == NULL) {
    fprintf(stderr, "FATAL ERROR: config->input is NULL in load_image_from_file.\\n");
    exit(EXIT_FAILURE);
  }
  fprintf(stderr, "DEBUG: In load_image_from_file, about to process file: %s\\n", config->input);
...
void visibility_host_set_up2(Config *config, float3 *vis_uvw_coords) {
  // Defensive check for the visibility source file
  if (config == NULL) {
    fprintf(stderr, "FATAL ERROR: config pointer is NULL in visibility_host_set_up2.\\n");
    exit(EXIT_FAILURE);
  }
  if (config->visibility_source_file == NULL) {
    fprintf(stderr, "FATAL ERROR: config->visibility_source_file is NULL in visibility_host_set_up2.\\n");
    exit(EXIT_FAILURE);
  }

  fprintf(stderr, "DEBUG: Loading visibility data from: %s\\n", config->visibility_source_file);
