...
inline float2 complex_mult(const float2 z1, const float2 z2) {
  float2 result;
...
half_support); ++v)
                ...
                // Correct the v index if necessary to respect the grid bounds
                        ...
                        corrected_v = MAX(-grid_center, MIN(grid_center - 1,
corrected_v));
...
grid_center;
                                ...
                                corrected_u = MAX(-grid_center, MIN(grid_center
- 1, corrected_u));
...
// void dump_inputs(int grid_size, int num_visibilities, int num_kernels,
//                  int total_kernels_samples, int oversampling_factor,
//                  int num_chunk, float2 *kernels, int2 *kernel_supports,
//                  float2 *input_grid, float3 *vis_uvw_coords, Config *config,
//                  int *iterator, float2 *output_visibilities) {
//   printf("kernels\n");
...
//   }
//   for (int i = 0; i < num_visibilities; i++) {
//     printf("%f %f %f\n", vis_uvw_coords[i].x, vis_uvw_coords[i].y,
//            vis_uvw_coords[i].z);
//   }
//   printf("config->w_scale: %lf\n", config->w_scale);
//   printf("config->uv_scale: %lf\n", config->uv_scale);
//   printf("iterator: %d\n", *iterator);
//   printf("precision: %d\n", sizeof(PRECISION));
...
void gridding_CPU(int NUM_VISIBILITIES, int GRID_SIZE, int NUM_KERNELS,
                  int TOTAL_KERNELS_SAMPLES, int OVERSAMPLING_FACTOR,
                  IN float2 *kernel, IN int2 *supports,
                  IN float3 *vis_uvw_coord, float2 *vis, IN Config *config,
                  IN float2 *grid_in, OUT float2 *grid) {
