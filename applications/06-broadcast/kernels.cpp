...
void b(int val[1]) {
  ...
  printf("Address = %lu\n", (size_t)&val[0]);
  int local_val = val[0];
  printf("b: Read val[0]=%d into local_val\n", local_val);
  mutex.unlock();
  printf("b: After unlock, about to check local_val=%d\n", local_val);
  if (local_val == 42) {
    printf("b: Incrementing success_count\n");
    success_count++;
  } else {
...
void c(int val[1]) {
  ...
  printf("Address = %lu\n", (size_t)&val[0]);
  int local_val = val[0];
  printf("c: Read val[0]=%d into local_val\n", local_val);
  mutex.unlock();
  printf("c: After unlock, about to check local_val=%d\n", local_val);
  if (local_val == 42) {
    printf("c: Incrementing success_count\n");
    success_count++;
  } else {
