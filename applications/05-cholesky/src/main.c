...
void blocked_cholesky_enkits_tasks(double *inout_matrix) {
  ...
  for (int k = 0; k < num_blocks; k++) {
    ...
    struct trsm_task_data *trsm_data =
        malloc(sizeof(struct trsm_task_data) * (num_blocks - k - 1));
    for (int i = k + 1; i < num_blocks; i++) {
    ...
    struct gemm_task_data *gemm_data =
        malloc(sizeof(struct gemm_task_data) * update_count);
    struct syrk_task_data *syrk_data =
        malloc(sizeof(struct syrk_task_data) * (num_blocks - k - 1));
...
int main(int argc, char *argv[]) {
  ...
  double convert_time =
      ((double)(convert_end.tv_sec - convert_start.tv_sec)) +
