...
# ---------------------------------------------------------------------------
def generate_scenario(num_cores: int, slam_name: str, image_width: int, image_height: int) -> str:
    cores = [f"Core{i}" for i in range(num_cores)]
...
{constraint_groups}    </constraints>
    ...
    <parameterValues>
        ...
        <parameter parent="Hextract" name="image_width" value="{image_width}" type="PARAMETER"/>
        <parameter parent="Hextract" name="image_height" value="{image_height}" type="PARAMETER"/>
    </parameterValues>
...
def main():
    ...
    parser.add_argument("--image-width", type=int, default=3840,
                        help="Image width in pixels (default: 3840)")
    parser.add_argument("--image-height", type=int, default=2160,
                        help="Image height in pixels (default: 2160)")
    ...
    (repo / "Scenarios" / scenario_name).write_text(
        generate_scenario(args.num_cores, slam_name, args.image_width, args.image_height)
    )
