...
// (FlattenPass.cpp:181-190), so each @emit sees its own value. Covers
// cross-hierarchy params + per-call-site specialization (the old dedup TODO).
...
iara.actor @run(%a: i64, %b: i64) {
  ...
  iara.node @sink in %x : i32
  %y = iara.node @inner params %b : i64 out i32
  iara.node @sink in %y : i32
